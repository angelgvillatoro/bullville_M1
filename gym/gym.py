...
a = pd.read_csv(
    "C:/Users/an199/OneDrive/pegos/gym/plantillas/planA_plantilla.csv",
    index_col=0,
)
a_rep = pd.read_csv(
    "C:/Users/an199/OneDrive/pegos/gym/plantillas/repA.csv",
    index_col=0,
)
b = pd.read_csv(
    "C:/Users/an199/OneDrive/pegos/gym/plantillas/planB_plantilla.csv",
    index_col=0,
)
b_rep = pd.read_csv(
    "C:/Users/an199/OneDrive/pegos/gym/plantillas/repB.csv",
    index_col=0,
)
rm = pd.read_csv(
    "C:/Users/an199/OneDrive/pegos/gym/plantillas/rm.csv", index_col=0
)
A_new = pd.DataFrame()
B_new = pd.DataFrame()
...
while i < 16:
    ...
    if 0 <= i < 6:
        if rm.index[i] == "roller":
            ...
            dlist = [A_new, row1 + row2]
            A_new = pd.concat(dlist)
            i = i + 1
            new_rm.append(A_new.loc["roller"].iloc[-1])
        else:
            new_rm.append(rm.iloc[i, len(rm.columns) - 1] + 2.5)
            row1 = (
                a[a.index.str.contains(rm.index[i])] * rm.iloc[i, len(rm.columns) - 1]
            ) + 2.5
            row2 = a_rep[a_rep.index.str.contains(rm.index[i])]
        ...
        if rm.index[i] == "roller_b":
            ...
            dlist = [B_new, row1 + row2]
            B_new = pd.concat(dlist)
            new_rm.append(A_new.loc["roller"].iloc[-1])
            i = i + 1
            ...
            dlist = [B_new, r1, r2]
            B_new = pd.concat(dlist)
            new_rm.append(B_new.loc["curl_nordico"].iloc[-1])
            new_rm.append(B_new.loc["curl_nordico_rep"].iloc[-1])
            i = i + 2
        elif (
            ...
            B_new = pd.concat(dlist)
            i = i + 1
new_rm = pd.DataFrame(new_rm, index=rm.index)
