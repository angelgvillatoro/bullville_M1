# Mejora del solver Navier-Stokes con condiciones de frontera mejoradas,
# paso de presión modularizado con SOR, ejecución paralela con multiprocessing,
# y guardado incremental para escalabilidad.
...
# ------------------------------------------------------------------------------------
# FUNCION: solucion_poisson_sor (Sustituye bucle de 20 pasos con SOR)
# ------------------------------------------------------------------------------------
@njit(fastmath=True)
def solucion_poisson_sor(p, rhs, omega, dx, dy, max_iter=100):
    ny, nx = p.shape
    for _ in range(max_iter):
        for i in range(1, ny - 1):
            for j in range(1, nx - 1):
                p_old = p[i, j]
                p_new = 0.25 * (p[i+1, j] + p[i-1, j] + p[i, j+1] + p[i, j-1] - rhs[i, j] * dx * dy)
                p[i, j] = (1 - omega) * p_old + omega * p_new
    return p

# ------------------------------------------------------------------------------------
# FUNCION PRINCIPAL: actualizar_campos (con SOR, condiciones mejoradas)
# ------------------------------------------------------------------------------------
...
def actualizar_campos(u_old, v_old, p_old, T_old, u_star, v_star, p_star, T_star,
    ...
    for j in prange(nx):
        x = j / (nx - 1)
        if x < 0.6:
            p_star[-1, j] = 1.0 - presion_factor * x
        else:
            p_star[-1, j] = 0.94 + presion_factor * (x - 0.6)**2
    ...
    rhs = np.zeros_like(p_star)
    for i in prange(1, ny - 1):
        for j in prange(1, nx - 1):
            rhs[i, j] = ((u_star[i, j + 1] - u_star[i, j - 1]) / (2 * dx_star) +
                         (v_star[i + 1, j] - v_star[i - 1, j]) / (2 * dy_star))
    p_star = solucion_poisson_sor(p_star, rhs, omega=1.7, dx=dx_star, dy=dy_star)
    ...
    tau = np.zeros((ny, nx))
    for j in prange(nx):
        tau[0, j] = (u_star[1, j] - u_star[0, j]) / dy_star

    # --- Condiciones de frontera restauradas (originales) ---
    v_star[0, :] = 0
    v_star[-1, :] = 0
    u_star[0, :] = -0.5
    u_star[-1, :] = 1.0
...
# --------------------------------------------------------------------------------------
# BLOQUE: Simulación completa y animación de resultados
# Contiene:
# - run_simulation: ejecución del solver de Navier-Stokes 2D
# - guardar_resultado: guardado de datos en disco
# - animar_resultados_linea: visualización de temperatura, velocidad, presión y esfuerzo
# - ciclo principal: ejecución en múltiples resoluciones y exportación de animaciones
# --------------------------------------------------------------------------------------
...
def run_simulation(nx=1600, ny=1600):
    ...
    # Historias de campos para animación
...
def guardar_resultado(sim_data, nx, ny, folder='sim_haller'):
